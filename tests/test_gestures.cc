...
int main() {
    g_feel.long_press_ms = 500;

    // Press, no detent, release under the threshold → short press.
    {
        PressState st;
        Check(Recognize(Down(Control::kMod, 0), st) == Gesture::kNone,
              "down emits nothing");
        Check(Recognize(Up(Control::kMod, 300), st) == Gesture::kPressShort,
    ...
    {
        PressState st;
        Recognize(Down(Control::kMod, 0), st);
        Check(Recognize(Up(Control::kMod, 600), st) == Gesture::kPressLong,
    ...
    {
        PressState st;
        Recognize(Down(Control::kEnc0, 0), st);
        Check(Recognize(Turn(Control::kEnc0, 1, 10), st) == Gesture::kHoldTurn,
              "detent at 10ms is a hold-turn");
        Check(Recognize(Up(Control::kEnc0, 300), st) == Gesture::kNone,
    ...
    {
        PressState st;
        Recognize(Down(Enc(1), 0), st);
        Check(Recognize(Turn(Enc(1), -1, 490), st) == Gesture::kHoldTurn,
              "detent at 490ms is a hold-turn");
        Check(Recognize(Up(Enc(1), 500), st) == Gesture::kNone,
    ...
    // Detent after release → a plain turn, no press state involved.
    {
        PressState st;
        Recognize(Down(Control::kEnc0, 0), st);
        Recognize(Up(Control::kEnc0, 300), st);
        Check(Recognize(Turn(Control::kEnc0, 1, 400), st) == Gesture::kTurn,
    ...
    {
        PressState st;
        Check(Recognize(Up(Control::kMod, 100), st) == Gesture::kNone,
    ...
    // is short.
    {
        PressState st;
        Recognize(Down(Control::kMod, 0), st);
        Check(Recognize(Up(Control::kMod, 500), st) == Gesture::kPressLong,
              "exactly 500ms is a long press");
    }
    {
        PressState st;
        Recognize(Down(Control::kMod, 0), st);
        Check(Recognize(Up(Control::kMod, 499), st) == Gesture::kPressShort,
    ...
    {
        PressState st;
        Check(Recognize(Turn(Control::kEnc0, 3, 0), st) == Gesture::kTurn,
