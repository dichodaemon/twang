...
///
/// The transport has silent-drop paths — MIDI ring full, channel filter,
...
/// lock.
struct LossCounters {
    std::atomic<std::uint32_t> midi_ring_full{0};       ///< cm85: MIDI ring Push() returned false
    ...
    void Reset() {
        midi_ring_full.store(0, std::memory_order_relaxed);
