...
/// structs — params, key-follow depth, and routes). `Set`/`SetRoute` write
/// `pending_` and set the `dirty_` flag without publishing; `Flush()` publishes
/// the whole set once (if dirty) by copying it into the back buffer and
/// advancing the monotonic `front_` counter (the published buffer index is
/// `front_ & 1`). The audio thread snapshots the front buffer into the parts at
/// each block boundary. A `reading_` flag lets the writer wait (spin, control
/// thread only) until the audio thread finishes reading a buffer before it
/// reuses that buffer — the audio thread never takes a lock (it may retry its
/// claim while the control thread publishes).
...
class ParamBlock {
  public:
    /// @brief Update one normalized parameter for a part.
    ...
    /// audio thread observes it all-or-nothing — never half-applied.
    /// Control thread only.
    void Flush();
  ...
  private:
    ...
    bool dirty_{false};                     ///< pending_ diverged from the published buffer (control thread only)
};
